# UptimeMonitor-Website

The official marketing website for the Uptime Monitor project - an open-source, self-hosted infrastructure monitoring solution.

> **Note:** This repository contains only the main website. For the actual monitoring software, please visit the repositories below.

## Related Projects

| Repository                                                                             | Description                                                                       |
| -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| [UptimeMonitor-Server](https://github.com/Rabbit-Company/UptimeMonitor-Server)         | Central monitoring server with REST API, WebSocket updates and ClickHouse storage |
| [UptimeMonitor-StatusPage](https://github.com/Rabbit-Company/UptimeMonitor-StatusPage) | Beautiful, customizable status page frontend with 15 themes                       |
| [PulseMonitor](https://github.com/Rabbit-Company/PulseMonitor)                         | Distributed monitoring agent supporting 16 protocols                              |

## Live Demo

- **Website:** [uptime-monitor.org](https://uptime-monitor.org)
- **Status Page Demo:** [status.passky.org](https://status.passky.org)

## License

[GPL-3.0](LICENSE)
